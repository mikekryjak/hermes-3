...
def run_manufactured_solutions_test(test_input):
   ...
   """
    ...
    for workdir in workdirs:
        ...
        datasets.append(
            open_boutdataset(
                boutmeshpath, inputfilepath=boutinppath, keep_yboundaries=False
            )
        )
    ...
    try:
        ...
        for key, variable_set in plot_data.items():
            ...
            plt.plot(
                xaxis,
                yaxis[0] * (xaxis / xaxis[0]) ** expected_slope,
                "x-",
            ...
            if fit is not None:
                plt.plot(
                    ...
                    fit,
                    "x-",
                    label="$e^{{{:.2f}}}\\Delta^{{{:.2f}}}$".format(offset, slope),
                )
    ...
    for varstring in function_list:
        ...
        for m in range(0, ntest):
            if varstring == "ddt(Pd)" and evolve_momentum and conservation_test:
                ...
                Vd = NVd / (Nd * mass)
                numerical = (3.0 / 2.0) * collectvar(
                    datasets, "ddt(Pd)", m
                ) + Vd * collectvar(datasets, "ddt(NVd)", m)
            ...
            if conservation_test:
                thisl2 = np.abs(
                    volume_integral(error_values, dx, dy, dz, J)
                    / volume_integral(normvar, dx, dy, dz, J)
                )
            else:
                thisl2 = np.sqrt(
                    volume_integral(error_values**2, dx, dy, dz, J)
